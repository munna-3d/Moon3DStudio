...
export default function WorkPageClient({ initialProjects = [] }: WorkPageClientProps) {
  const [activeFilter, setActiveFilter] = useState<FilterCategory>("ALL");
  const [selectedTechProject, setSelectedTechProject] = useState<Project | null>(null);

  const displayProjects = initialProjects.length > 0 ? initialProjects : STATIC_PROJECTS;
  ...
  return (
    <div className="pt-32 pb-24 bg-[#090a0d] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Filter Pills */}
        <div className="flex flex-wrap items-center gap-2 mb-10 pb-6 border-b border-white/10" role="toolbar" aria-label="Portfolio category filter">
            ...
            >
              ...
              >
                <Image
                  ...
                  alt={`${project.title} — ${project.categoryLabel} 3D model created by Moon 3D Studio`}
                  fill
        ...
        <div className="p-8 md:p-12 rounded-2xl bg-[#11141a] border border-white/10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="font-display font-bold text-xl sm:text-2xl text-white uppercase mb-2">
